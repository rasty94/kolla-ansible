...
``http://<kolla_internal_vip_address>:<opensearch_dashboards_server_port>`` or
``http://<kolla_external_vip_address>:<opensearch_dashboards_server_port>``. Authenticate
using ``<opensearch_dashboards_user>`` and ``<opensearch_dashboards_password>``.

The values ``<kolla_internal_vip_address>``, ``<kolla_external_vip_address>``
``<opensearch_dashboards_server_port>`` and ``<opensearch_dashboards_user>`` can be found in
...
``<opensearch_dashboards_password>`` can be found in ``/etc/kolla/passwords.yml``.
